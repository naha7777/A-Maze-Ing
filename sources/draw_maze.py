    ...
    for y0 in range(height):
        for x0 in range(width):
            ...
            if (x0, y0) in color_ft:
                pygame.draw.rect(screen, BLUE, (cell_x-20, cell_y-20, cell,
                                                cell))
            if x0 == x1 and y0 == y1:
                pygame.draw.rect(screen, GREEN, (cell_x-20, cell_y-20, cell,
                                                 cell))
            if x0 == x2 and y0 == y2:
                pygame.draw.rect(screen, RED, (cell_x-20, cell_y-20, cell,
                                               cell))


def print_path(cell_walls: list, width: int, height: int, cell: int,
               screen: pygame.Surface, x: int, y: int, x1: int, y1: int,
               x2: int, y2: int, color_ft: list, path_coordinates: list,
               color: tuple) -> None:
